...
function gameCardHTML(g) {
  ...
  return `<div class="gamecard">
    ...
    <div id="${g}-body" class="gcbody">
      <div class="fr2">
        <div><label class="fl-lbl">Opening Float (KES)</label><input type="number" class="fl-inp" id="${g}-open" placeholder="0" oninput="if(this.value<0)this.value=0;S.shopData[activeShop].games[this.id.replace('-open','').replace('-close','')][this.id.endsWith('-open')?'open':'close']=N(this.value);recalcDebounced();" onblur="if(this.value<0)this.value=0;saveShopState(activeShop)" min="0"></div>
        <div><label class="fl-lbl">Closing Float (KES)</label><input type="number" class="fl-inp" id="${g}-close" placeholder="0" oninput="if(this.value<0)this.value=0;recalcDebounced();" onblur="if(this.value<0)this.value=0;saveShopState(activeShop)" min="0"></div>
      </div>
...
function renderExpList() {
  ...
  exps.forEach((e, i) => {
    ...
    a.oninput = () => { if(a.value<0) a.value=0; S.shopData[activeShop].expenses[i].amount = parseFloat(a.value) || 0; recalcDebounced(); };
