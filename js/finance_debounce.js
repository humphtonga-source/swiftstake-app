// Debounced recalc - waits 600ms after last keystroke before updating DOM
function recalcDebounced() {
  clearTimeout(window._recalcTimer);
  window._recalcTimer = setTimeout(recalc, 600);
}
...
function renderFinance() {
  ...
  <div class="card" style="border-color:rgba(245,158,11,0.3);">
    ...
    <input type="number" id="opening-cash-inp" class="fl-inp" placeholder="e.g. 5000" min="0" style="font-size:18px;font-weight:800;color:var(--gold);border-color:rgba(245,158,11,0.4);" oninput="if(this.value<0)this.value=0;S.shopData[activeShop].openingCash=N(this.value);recalcDebounced();" onblur="if(this.value<0)this.value=0;S.shopData[activeShop].openingCash=N(this.value);saveShopState(activeShop);">
...
function gameCardHTML(g) {
  ...
  return `<div class="gamecard">
    ...
    <div id="${g}-body" class="gcbody">
      <div class="fr2">
        <div><label class="fl-lbl">Opening Float (KES)</label><input type="number" class="fl-inp" id="${g}-open" placeholder="0" oninput="if(this.value<0){this.value=0;alert('Opening float cannot be negative');}recalcDebounced()" onblur="if(this.value<0)this.value=0;saveShopState(activeShop)" min="0"></div>
        <div><label class="fl-lbl">Closing Float (KES)</label><input type="number" class="fl-inp" id="${g}-close" placeholder="0" oninput="if(this.value<0){this.value=0;alert('Closing float cannot be negative');}recalcDebounced()" onblur="if(this.value<0)this.value=0;saveShopState(activeShop)" min="0"></div>
      </div>
...
function renderExpList() {
  ...
  exps.forEach((e, i) => {
    ...
    a.oninput = () => { if(a.value<0){a.value=0;alert('Expense amount cannot be negative');} S.shopData[activeShop].expenses[i].amount = parseFloat(a.value) || 0; recalcDebounced(); };
