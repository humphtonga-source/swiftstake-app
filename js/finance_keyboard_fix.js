...
async function saveShopState(shop, skip) {
  ...
  const d = S.shopData[shop]; if (!d) return;
  try {
    const p = {
      shop, games: d.games,
...
function renderFinance() {
  ...
  <div class="card" style="border-color:rgba(245,158,11,0.3);">
    ...
    <input type="number" id="opening-cash-inp" class="fl-inp" placeholder="e.g. 5000" min="0" style="font-size:18px;font-weight:800;color:var(--gold);border-color:rgba(245,158,11,0.4);" oninput="if(this.value<0)this.value=0;S.shopData[activeShop].openingCash=N(this.value);recalc();" onblur="if(this.value<0)this.value=0;S.shopData[activeShop].openingCash=N(this.value);saveShopState(activeShop);">
  ...
  <div class="reconcard">
    ...
    <div class="recongrid" style="grid-template-columns:1fr 1fr;">
      ...
      <div class="reconmc" style="background:var(--surface2);border:1px solid var(--border);grid-column:span 2;"><div class="fl-lbl">🎯 Expected Float (KES)</div><div id="recon-expected" style="font-size:20px;font-weight:800;color:var(--txt);margin-top:8px;">KES 0</div><div id="recon-diff" style="font-size:12px;font-weight:700;margin-top:6px;color:var(--txt3);">Enter amounts to check</div></div>
    </div>
    <button onclick="doRecon()" style="width:100%;margin-top:10px;padding:12px;background:var(--gold);color:#111;font-weight:700;font-size:15px;border:none;border-radius:var(--radius2);cursor:pointer;">🔍 Check Reconciliation</button>
...
function gameCardHTML(g) {
  ...
  return `<div class="gamecard">
    ...
    <div id="${g}-body" class="gcbody">
      <div class="fr2">
        <div><label class="fl-lbl">Opening Float (KES)</label><input type="number" class="fl-inp" id="${g}-open" placeholder="0" oninput="if(this.value<0){this.value=0;alert('Opening float cannot be negative');}recalc()" onblur="if(this.value<0)this.value=0;saveShopState(activeShop)" min="0"></div>
        <div><label class="fl-lbl">Closing Float (KES)</label><input type="number" class="fl-inp" id="${g}-close" placeholder="0" oninput="if(this.value<0){this.value=0;alert('Closing float cannot be negative');}recalc()" onblur="if(this.value<0)this.value=0;saveShopState(activeShop)" min="0"></div>
      </div>
...
function saveInputs() {
  ...
  const oci = $('opening-cash-inp'); if (oci && oci.value !== '') S.shopData[activeShop].openingCash = N(oci.value);
  GAMES.forEach(g => {
    const oi = $(`${g}-open`), ci = $(`${g}-close`);
    if (!S.shopData[activeShop].games[g]) S.shopData[activeShop].games[g] = {open:0, close:0, topups:[]};
    if (oi && oi.value !== '') S.shopData[activeShop].games[g].open  = N(oi.value);
    if (ci && ci.value !== '') S.shopData[activeShop].games[g].close = N(ci.value);
  });
  const rows = document.querySelectorAll('#explist .exprow');
  rows.forEach((row, i) => {
    if (S.shopData[activeShop].expenses[i]) {
      const d = row.querySelector('input[type="text"]'), a = row.querySelector('input[type="number"]');
      if (d) S.shopData[activeShop].expenses[i].desc   = d.value;
      if (a) S.shopData[activeShop].expenses[i].amount = parseFloat(a.value) || 0;
    }
  });
}
...
function loadShopData(shop) {
  ...
  requestAnimationFrame(() => {
    const oci = $('opening-cash-inp'); if (oci) oci.value = (d.openingCash && d.openingCash > 0) ? d.openingCash : '';
    GAMES.forEach(g => {
...
async function switchShop(el, shop) {
  ...
  saveInputs(); await saveShopState(activeShop);
  ...
  const openedAt = $('opened-at-display'); if (openedAt) openedAt.textContent = '';
  ['s-rev','s-exp','eod-ocash','eod-topup','eod-rev','eod-exp','eod-net'].forEach(id => { const e = $(id); if (e) e.textContent = 'KES 0'; });
  const sn = $('s-net'); if (sn) { sn.textContent = 'KES 0'; sn.className = 'mcval'; }
  await refreshShopData(shop);
  _resetting = false; renderFinance(); setTimeout(() => loadShopData(shop), 80);
}
...
async function refreshShopData(shop) {
  try {
    ...
    if (!S.shopData[shop]) S.shopData[shop] = {games:{stellar:{open:0,close:0,topups:[]},pilot:{open:0,close:0,topups:[]},spin:{open:0,close:0,topups:[]}}, expenses:[], openingCash:0, cashRecon:null};
...
async function confirmCashMov() {
  ...
  hideCashMovForm(); renderCashMovLog(); updateCashMovEOD(); saveShopState(activeShop);
...
async function removeCashMov(i) {
  ...
  renderCashMovLog(); updateCashMovEOD(); saveShopState(activeShop);
...
function recalc() {
  saveInputs(); const d = S.shopData[activeShop]; let tO = 0, tT = 0, tW = 0, tR = 0;
  GAMES.forEach(g => {
    if (!d.games[g]) d.games[g] = {open:0, close:0, topups:[]};
  ...
  const tExp = (d.expenses || []).reduce((s,e) => s + N(e.amount), 0), ocash = N(d.openingCash), net = ocash + tR - tExp;
...
function doRecon() {
  const cEl = $('recon-cash'), mEl = $('recon-mpesa');
  const expEl = $('recon-expected'), diffEl = $('recon-diff'), resEl = $('recon-result');
  if (!expEl) return;
  const d = S.shopData[activeShop];
  if (!d) return;
  const opening = N(d.openingCash);
  const cash = N(cEl ? cEl.value : 0), mpesa = N(mEl ? mEl.value : 0);
  let gameRev = 0;
  GAMES.forEach(g => {
    const gd = d.games[g] || {}, topupsArr = gd.topups || [];
    const nets = topupsArr.reduce((s,t) => s + N(t.amount), 0);
    gameRev += (N(gd.open) + nets) - N(gd.close);
  });
  const expenses = (d.expenses || []).reduce((s,e) => s + N(e.amount), 0);
  const expected = opening + gameRev - expenses;
  const declared = cash + mpesa, diff = declared - expected, ok = Math.abs(diff) < 1;
  d.cashRecon = {opening, cash, mpesa, gameRev, expenses, expected, declared, diff, ok};
  expEl.textContent = 'KES ' + fmt(expected);
  const anyFilled = (cEl && cEl.value !== '') || (mEl && mEl.value !== '');
  if (!anyFilled) {
    if (diffEl) diffEl.textContent = 'Enter amounts and tap Check';
  ...
  if (resEl) {
    resEl.innerHTML = `<div style="border:1px solid ${ok?'rgba(34,197,94,0.3)':'rgba(239,68,68,0.3)'};border-radius:var(--radius2);overflow:hidden;margin-top:6px;">
      <div style="background:${ok?'var(--greenl)':'var(--redl)'};padding:14px 16px;display:flex;align-items:center;gap:12px;">
        <span style="font-size:32px;">${ok?'✅':'⚠️'}</span>
        <div>
          <div style="font-size:15px;font-weight:700;color:${ok?'var(--green)':'var(--red)'};">${ok?'Balanced — all money accounted for':diff>0?'KES '+fmt(diff)+' OVER':'KES '+fmt(Math.abs(diff))+' SHORT'}</div>
          <div style="font-size:12px;color:var(--txt2);margin-top:3px;">Expected: <b>KES ${fmt(expected)}</b> · Declared: <b>KES ${fmt(declared)}</b></div>
        </div>
      </div>
    </div>`;
  }
...
async function pushSection(section) {
  const safeId = section.replace(/:/g, '-');
  const statusEl = document.getElementById('push-' + safeId + '-status');
  const btnEl    = document.getElementById('push-' + safeId + '-btn');
  function setStatus(msg, color) { if (statusEl) { statusEl.textContent = msg; statusEl.style.color = color || 'var(--txt3)'; } }
  function setBtnState(loading)  { if (btnEl) { btnEl.disabled = loading; btnEl.style.opacity = loading ? '0.5' : '1'; } }
  setStatus('⏳ Pushing...', 'var(--blue)');
  setBtnState(true);
  try {
    saveInputs();
    const d = S.shopData[activeShop]; if (!d) throw new Error('No shop data for ' + activeShop);

    // ── AUDIT: capture state BEFORE the push ──
    const beforeSnap = AuditLog.snapshot(section, d);

    let patch = {updated_at: new Date().toISOString()};
    if (section === 'opening') {
      patch.opening_cash = d.openingCash || 0;
      patch.opened_at    = d.openedAt    || null;
    } else if (section === 'expenses') {
      patch.expenses = Array.isArray(d.expenses) ? d.expenses : [];
    } else if (section === 'recon') {
      doRecon();
      patch.cash_recon = d.cashRecon || null;
    } else if (section.startsWith('game:')) {
      const allGames = {};
      GAMES.forEach(gn => { allGames[gn] = d.games[gn] || {open:0, close:0, topups:[]}; });
      patch.games = allGames;
    } else {
    ...
    const {error: upsertErr} = await db.from('shop_state').upsert(upsertPayload, {onConflict:'shop'});
    if (upsertErr) throw new Error(upsertErr.message || 'Save failed');

    // ── AUDIT: capture state AFTER push and record ──
    const afterSnap = AuditLog.snapshot(section, d);
    await AuditLog.record('push', activeShop, section, beforeSnap, afterSnap);

    setStatus('✅ ' + new Date().toLocaleTimeString('en-KE',{hour:'2-digit', minute:'2-digit'}), 'var(--green)');
    pushNotif('🔄 ' + activeShop + ' pushed', 'Section: ' + section.replace('game:', 'Game — '));
  } catch(e) {
    setStatus('❌ ' + (e.message || 'Error'), 'var(--red)');
  } finally {
    setBtnState(false);
...
function renderExpList() {
  ...
  const exps = S.shopData[activeShop].expenses; el.innerHTML = '';
  exps.forEach((e, i) => {
    const row = document.createElement('div'); row.className = 'exprow';
    const d = document.createElement('input'); d.type = 'text'; d.placeholder = 'Description'; d.value = e.desc || ''; d.id = 'exp-desc-' + i;
    d.oninput  = () => { S.shopData[activeShop].expenses[i].desc = d.value; clearTimeout(d._t); d._t = setTimeout(() => saveShopState(activeShop), 800); };
    d.onblur   = () => { S.shopData[activeShop].expenses[i].desc = d.value; saveShopState(activeShop); };
    const a = document.createElement('input'); a.type = 'number'; a.placeholder = 'KES'; a.value = e.amount || ''; a.style.maxWidth = '110px';
    a.oninput = () => { if(a.value<0){a.value=0;alert('Expense amount cannot be negative');} S.shopData[activeShop].expenses[i].amount = parseFloat(a.value) || 0; recalc(); };
    a.onblur  = () => { if(a.value<0)a.value=0; S.shopData[activeShop].expenses[i].amount = parseFloat(a.value) || 0; saveShopState(activeShop); };
    const btn = document.createElement('button'); btn.className = 'delbtn'; btn.textContent = '✕'; btn.onclick = () => delExp(i);
    row.appendChild(d); row.appendChild(a); row.appendChild(btn); el.appendChild(row);
  });
}
function delExp(i) {
  const beforeSnap = AuditLog.snapshot('expenses', S.shopData[activeShop]);
...
async function submitReport() {
  ...
  saveInputs(); const d = S.shopData[activeShop];
  ...
  const tExp = expenses.reduce((s,e) => s + e.amount, 0), net = ocash + tR - tExp;
  ...
  try { 
    const {error} = await db.from('reports').insert({
      ...
      games:gs, expenses, cash_recon:d.cashRecon||null, 
      totals:{openingCash:ocash, topup:tT, revenue:tR, expenses:tExp, net}
    }); 
    if (error) throw error;
  } catch(e) {
  ...
  // ── AUDIT: record EOD submission ──
  await AuditLog.record('submit', activeShop, 'end-of-day',
    'daily state cleared',
    `Report #${reportId} by ${sess.name} | Net KES ${fmt(net)} | Revenue KES ${fmt(tR)} | Expenses KES ${fmt(tExp)}`
  );
  
  // Always save to local state (works offline)
  S.reports.unshift({id:reportId, shop:activeShop, date:dateStr, time:timeStr, by:sess.name, games:gs, expenses, cashRecon:d.cashRecon||null, totals:{openingCash:ocash, topup:tT, revenue:tR, expenses:tExp, net}});
  ...
  setTimeout(() => {
    loadShopData(activeShop);
    // Explicitly clear recon inputs — they are static HTML and don't reset on their own
    const rc = $('recon-cash'), rm = $('recon-mpesa');
    const re = $('recon-expected'), rd = $('recon-diff'), rr = $('recon-result');
    if (ro) ro.value = '';
    if (rc) rc.value = '';
...
function exportCurrent() {
  saveInputs(); const d = S.shopData[activeShop]; const now = new Date(); const gs = {}; let tR = 0;
  ...
  const r = {shop:activeShop, date:now.toLocaleDateString('en-KE'), time:now.toLocaleTimeString('en-KE'), by:sess.name, games:gs, expenses, cashRecon:d.cashRecon||null, totals:{openingCash:ocash, revenue:tR, expenses:tExp, net:ocash+tR-tExp}};
