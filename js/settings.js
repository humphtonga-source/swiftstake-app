...
function toggleNotifPref(key, enabled) {
  ...
  const track = document.getElementById(key + '_track');
  const thumb = document.getElementById(key + '_thumb');
  if (track) track.style.background = enabled ? 'var(--gold)' : 'var(--bg2)';
...
function syncNotifToggles() {
  ['notif_reports','notif_cash','notif_chat','notif_opening'].forEach(key => {
...
async function requestNotifPermission() {
  ...
  if (!('Notification' in window)) {
    if (status) { status.textContent = '\u26a0\ufe0f Notifications not supported on this browser.'; status.style.color = 'var(--red)'; }
    return;
  }
  const perm = await Notification.requestPermission();
  if (perm === 'granted') {
    if (status) { status.textContent = '\u2705 Push notifications enabled!'; status.style.color = 'var(--green)'; }
    if (typeof initPushNotifications === 'function') {
      initPushNotifications(sess.name, sess.shop || 'admin');
    }
  } else {
