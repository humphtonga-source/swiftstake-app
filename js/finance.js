...
function doRecon() {
  ...
  const opening = N(d && d.openingCash);
  const cash = N(cEl ? cEl.value : 0), mpesa = N(mEl ? mEl.value : 0);
  let gameRev = 0;
  GAMES.forEach(g => { const gd = (d && d.games[g]) || {}, topupsArr = gd.topups || [], nets = topupsArr.reduce((s,t) => s + N(t.amount), 0); gameRev += (N(gd.open) + nets) - N(gd.close); });
  const expenses = (d && d.expenses || []).reduce((s,e) => s + N(e.amount), 0), expected = opening + gameRev - expenses, declared = cash + mpesa, diff = declared - expected, ok = Math.abs(diff) < 1;
  if (d) d.cashRecon = {opening, cash, mpesa, gameRev, expenses, expected, declared, diff, ok};
  ...
  if (resEl.innerHTML !== reconHTML) resEl.innerHTML = reconHTML;
}
