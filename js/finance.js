...
function renderFinance() {
  ...
  <div class="reconcard">
    ...
    <div class="recongrid">
      <div class="reconmc" style="background:var(--goldl);border:1px solid rgba(245,158,11,0.2);"><div class="fl-lbl" style="color:var(--gold);">☀️ Opening Cash (KES)</div><div id="recon-opening" style="margin-top:8px;font-size:18px;font-weight:800;color:var(--gold);">KES 0</div><div style="font-size:11px;color:var(--txt3);margin-top:2px;">Auto-filled from above</div></div>
...
function loadShopData(shop) {
  ...
  requestAnimationFrame(() => {
    ...
    if (d.cashRecon) {
      if (ro) ro.textContent = "KES " + fmt(d.openingCash || 0);
...
function doRecon() {
  ...
  if (!oEl || !expEl) return;
  const opening = N(S.shopData[activeShop].openingCash), cash = N(cEl ? cEl.value : 0), mpesa = N(mEl ? mEl.value : 0), d = S.shopData[activeShop];
  let gameRev = 0;
