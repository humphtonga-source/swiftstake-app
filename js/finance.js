...
function recalc() {
  ...
  // ✅ FIX: Include cash movements in net calculation
  const cashMov = S.shopData[activeShop].cashMovements || [];
  ...
  // Include cash movements in net: opening + revenue + added cash - withdrawn cash - expenses
...
function doRecon() {
  ...
  // ✅ FIX: Include cash movements in reconciliation
  const cashMov = d.cashMovements || [];
  const totalCashAdded = cashMov.filter(m => m.amount > 0).reduce((s,m) => s + m.amount, 0);
  const totalCashWithdrawn = Math.abs(cashMov.filter(m => m.amount < 0).reduce((s,m) => s + m.amount, 0));
  
  const expenses = (d.expenses || []).reduce((s,e) => s + N(e.amount), 0);
  // expected = opening + gameRev + added cash - withdrawn cash - expenses
