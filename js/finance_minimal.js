...
function loadShopData(shop) {
  ...
  requestAnimationFrame(() => {
    ...
    if (d.cashRecon) {
      if (ro && d.cashRecon.opening) ro.value = d.cashRecon.opening;
...
async function confirmCashMov() {
  ...
  S.shopData[activeShop].openingCash = Math.max(0, (S.shopData[activeShop].openingCash || 0) + (isAdd ? amt : -amt));
  const _oci = $('opening-cash-inp'); if (_oci) { _oci.value = S.shopData[activeShop].openingCash; }
  const _ro = $('recon-opening'); if (_ro) { _ro.value = S.shopData[activeShop].openingCash; }
...
async function removeCashMov(i) {
  ...
  const removedAmt = removed ? N(removed.amount) : 0;
  S.shopData[activeShop].cashMovements.splice(i, 1);
  S.shopData[activeShop].openingCash = Math.max(0, (S.shopData[activeShop].openingCash || 0) - removedAmt);
  const _ociR = $('opening-cash-inp'); if (_ociR) { _ociR.value = S.shopData[activeShop].openingCash; }
  const _roR = $('recon-opening'); if (_roR) { _roR.value = S.shopData[activeShop].openingCash; }
