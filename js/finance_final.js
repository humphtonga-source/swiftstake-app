// Helper: update recon-opening without stealing focus
function _updateReconOpening(val) {
  const ro = $('recon-opening');
  if (ro && document.activeElement !== ro) {
    ro.value = val || 0;
    doRecon();
  }
}
...
function renderFinance() {
  ...
  <div class="card" style="border-color:rgba(245,158,11,0.3);">
    ...
    <input type="number" id="opening-cash-inp" class="fl-inp" placeholder="e.g. 5000" min="0" style="font-size:18px;font-weight:800;color:var(--gold);border-color:rgba(245,158,11,0.4);" oninput="if(this.value<0)this.value=0;S.shopData[activeShop].openingCash=N(this.value);_updateReconOpening(N(this.value));clearTimeout(window._recalcT);window._recalcT=setTimeout(recalc,300);" onblur="if(this.value<0)this.value=0;S.shopData[activeShop].openingCash=N(this.value);saveShopState(activeShop);">
  ...
  <div class="reconcard">
    ...
    <div class="recongrid">
      <div class="reconmc" style="background:var(--goldl);border:1px solid rgba(245,158,11,0.2);"><div class="fl-lbl" style="color:var(--gold);">☀️ Opening Cash (KES)</div><input type="number" id="recon-opening" class="fl-inp" placeholder="0" min="0" oninput="doRecon()" style="margin-top:6px;font-size:18px;font-weight:800;border-color:rgba(245,158,11,0.4);background:var(--bg2);"></div>
...
function loadShopData(shop) {
  ...
  requestAnimationFrame(() => {
    ...
    const ro = $('recon-opening'), rc = $('recon-cash'), rm = $('recon-mpesa');
    if (d.cashRecon) {
      if (ro) ro.value = d.openingCash || 0;
      if (rc && d.cashRecon.cash)    rc.value = d.cashRecon.cash;
      if (rm && d.cashRecon.mpesa)   rm.value = d.cashRecon.mpesa;
    } else {
      if (ro) ro.value = (S.shopData[activeShop] && S.shopData[activeShop].openingCash) || 0;
      if (rc) rc.value = '';
      if (rm) rm.value = '';
...
async function confirmCashMov() {
  ...
  // Update opening cash based on movement
  const _movAmt = isAdd ? amt : -amt;
  S.shopData[activeShop].openingCash = Math.max(0, (S.shopData[activeShop].openingCash || 0) + _movAmt);
  const _oci = $('opening-cash-inp'); if (_oci) _oci.value = S.shopData[activeShop].openingCash;
  _updateReconOpening(S.shopData[activeShop].openingCash);
...
async function removeCashMov(i) {
  ...
  const _removedAmt = removed ? N(removed.amount) : 0;
  S.shopData[activeShop].cashMovements.splice(i, 1);
  // Reverse the movement from opening cash
  S.shopData[activeShop].openingCash = Math.max(0, (S.shopData[activeShop].openingCash || 0) - _removedAmt);
  const _ociR = $('opening-cash-inp'); if (_ociR) _ociR.value = S.shopData[activeShop].openingCash;
  _updateReconOpening(S.shopData[activeShop].openingCash);
...
function doRecon() {
  const oEl = $('recon-opening'), cEl = $('recon-cash'), mEl = $('recon-mpesa'), expEl = $('recon-expected'), diffEl = $('recon-diff'), resEl = $('recon-result');
  if (!oEl || !expEl) return;
  const opening = N(oEl.value), cash = N(cEl ? cEl.value : 0), mpesa = N(mEl ? mEl.value : 0), d = S.shopData[activeShop];
  let gameRev = 0;
  ...
  const expenses = (d.expenses || []).reduce((s,e) => s + N(e.amount), 0), expected = opening + gameRev - expenses, declared = cash + mpesa, diff = declared - expected, ok = Math.abs(diff) < 1;
  d.cashRecon = {opening, cash, mpesa, gameRev, expenses, expected, declared, diff, ok};
  if (expEl) expEl.textContent = 'KES ' + fmt(expected);
  const anyFilled = oEl.value !== '' || (cEl && cEl.value !== '') || (mEl && mEl.value !== '');
