  ...
  realtimeSub = db.channel('messages-channel')
    .on('postgres_changes', {event:'INSERT', schema:'public', table:'messages'}, payload => {
      ...
      if (!channels[ch].find(x => x._dbid === m.id)) {
        ...
        else if(notifEnabled('notif_chat')) pushNotif('💬 New message in #' + ch, (m.author || '') + (m.text ? ' — ' + m.text.substring(0, 40) : ''));
      }
    })
    .subscribe();
}
...
function subscribeToDataChanges() {
  ...
  realtimeDataSub = _rdb.channel('data-changes')
    .on('postgres_changes', {event:'*', schema:'public', table:'shop_state'}, payload => {
      ...
      // Admin: fully reload and re-render finance for the active shop
      if (sess.isAdmin && financeOpen && activeShop === sh) {
        refreshShopData(sh).then(() => {
          loadShopData(sh);
          renderFinance();
        });
      }
      ...
      if (!sess.isAdmin && sess.shop === sh && financeOpen) {
        ...
        // Only notify cashier once every 2 minutes per shop
        const cmKey = '_lastCMNotif_' + sh, now2 = Date.now();
        if (!window[cmKey] || now2 - window[cmKey] > 120000) {
          window[cmKey] = now2;
          if(notifEnabled('notif_cash')) pushNotif('💵 Cash movement updated', 'Admin updated ' + sh + ' cash');
        }
      }
      ...
      // Only notify admin once every 5 minutes per shop (was 30 seconds)
      const nKey = '_lastDataNotif_' + sh, now = Date.now();
      if (sess.isAdmin && (!window[nKey] || now - window[nKey] > 300000)) {
        window[nKey] = now;
        if(notifEnabled('notif_reports')) pushNotif('🔄 ' + sh + ' updated', 'Live data received from cashier');
      }
    })
    .on('postgres_changes', {event:'INSERT', schema:'public', table:'reports'}, payload => {
      ...
      if (sess.isAdmin) {
        ...
        if(notifEnabled('notif_reports')) pushNotif('📋 New report — ' + r.shop, 'Net KES ' + (r.totals && r.totals.net != null ? fmt(r.totals.net) : '—'));
      }
    })
    .subscribe();
}
